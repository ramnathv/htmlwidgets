...
#'(e.g. Browser, RStudio Viewer, R Markdown, Shiny). Note that typically widgets
#'can accept the default sizing policy (or override only one or two aspects of
#'it) and get satisfactory sizing behavior via the automatic sizing logic built
#'into the htmlwidgets framework (see the notes below for the most typical
#'exceptions to this).
#'
...
#'@param viewer.defaultWidth The default width used to display the widget within
#'  the RStudio Viewer.
...
#'  parameter specifies the default height for viewing in all contexts (browser,
#'  viewer, and knitr) unless it is specifically overridden with e.g.
...
#'@param padding Padding around the widget (in pixels). This parameter specifies
#'  the padding for viewing in all contexts (browser and viewer) unless it is
#'  specifically overrriden by e.g. \code{browser.padding}.
