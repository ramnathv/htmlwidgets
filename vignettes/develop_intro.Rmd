...
To start with we'll walk through the creation of a simple widget that wraps the [sigma.js](https://www.sigmajs.org/) graph visualization library. When we're done we'll be able to use it to display interactive visualizations of [GEXF](http://gexf.net/format/) (Graph Exchange XML Format) data files. For example:
