...
* Dynamically attaching dependencies to widgets. This might be a data file that is only used for some instances of the widget (e.g. a map projection).
...
Typically the HTML "housing" for a widget is just a `<div>` element, and this is correspondingly the default behavior for new widgets that don't specify otherwise. In the [introductory vignette](htmlwidgets-intro.Rmd) we built a wrapper for the [jQuery Knob](http://anthonyterrien.com/knob/) library. Since the knob widget uses an `<input>` element rather than a `<div>`, we need to write an R function `knob_html` which  provides us with the correct element type. Details on doing this are covered in the 
