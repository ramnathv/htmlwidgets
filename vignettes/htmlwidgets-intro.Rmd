...
Below we'll walk through an example widget that wraps the [sigma.js](http://sigmajs.org) graph visualization library.

#### File Layout

Let's assume that our widget is named **sigma** and is located within an R package of the same name. Our widget will read [GEXF](http://gexf.net) data files so we'll need to include both the base sigma.js library as well as it's GEXF plugin. Here are the files that we'll add to the package:
...
Let's try it out with some sample data included with the package:
