...
* [HTML Widget Sizing](sizing.Rmd) explains custom sizing policies and when you might need to use them and describes implementing a `resize` method within JavaScript bindings.

* [HTML Widgets: Advanced Topics](advanced.Rmd) describes framework features that support per-widget instance data, data tranformations (e.g. converting a data frame into a d3 dataset), and providing widget options that are live JavaScript objects (e.g. function definitions).
