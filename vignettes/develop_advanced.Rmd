...
Many htmlwidgets allow the user to alter the appearance of the widget using the user's mouse.  Ordinarily these changes are lost when the web page is closed.  However if a htmlwidget wants its state preserved then it can use the `stateChanged` function that is passed as an extra parameter to its `factory` function.  Anything passed to this function will be given back to the object when it is next loaded, via an extra `state` parameter given to `renderValue`.
